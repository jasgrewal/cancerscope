...
## Reference data for the rest of the script
import models_nold
...
def getargs():
	...
	parser_test.add_argument('-f', '--infile', nargs=1, type=proper_file, default=None, required=False, help='Specify the input file with samples. Order is gene name(hugo) \t sample1 \t sample2...')
	parser_test.add_argument('-t', '--transpose', action='store_true', default=False, required=False, help='If your file has the opposite layout (sample x gene), pass this flag')
	parser_test.add_argument('-d', '--delim', nargs=1, type=proper_delim, default=['\t'], required=False, help='Delimiter for input file, if not tabs')
	...
	parser_test.add_argument('-p', '--pref', nargs=1, type=str, required=True, help="specify prefix for output files")
	parser_test.add_argument('-x', '--makeblind', nargs=1, type=restricted_float, default=[0.0], help="Fraction of genes to randomly set to 0.0")
	parser_test.add_argument('-g', '--genecol', nargs=1, type=proper_genecol, default=["ENSG"], required=False, help='Specify the gene name type in the first column. Script assumes Columns 2 onwards are samples. Options for genecol include ENSG,HUGO, GENCODE, and GAF (not case sensitive)') 
	parser_test.add_argument('-m', '--multicolgenes', action='store_true', default='False', required=False, help='If you file has first col ENSG, second col HUGO, and then sample columns, pass this flag') 
