...
import os, sys, timeit, argparse, json, shutil
import json,csv
...
from argtypes import restricted_float, proper_file, proper_dir, Dictionary, check_balance_type
...
def getargs():
	parser = argparse.ArgumentParser(description="Receive input file and params for MLP")
	subparsers = parser.add_subparsers(dest='subcommand')
	
	#Subparser for testing (with passed architecture)
	parser_test = subparsers.add_parser('test')
	parser_test.add_argument('-f', '--infile', nargs=1, type=str, default=[None], required=False, help='Specify the input file with samples. Order is gene name(ensg) \t gene name (hugo) \t sample1 \t sample2...')
	parser_test.add_argument('-o', '--outdir', nargs=1, type=proper_dir, required=True, help='Specify the output directory. A timestamped subdir will be created for all outputs from this run')
	parser_test.add_argument('-n', '--indir', nargs=1, type=proper_file, required=True, help='Directory with bestmodel.npz and other files required from lasagna full train set run')
	parser_test.add_argument('-p', '--pref', nargs=1, type=str, required=True, help="specify prefix")
	parser_test.add_argument('-x', '--makeblind', nargs=1, type=float, default=[0.0])
...
def readtestinput(datfile, trimlabs, header_status, firstcol_ensg=True, sep='\t'):
	...
	if (datfile.endswith('.pkl')):
		...
		with open(datfile, 'r') as infile:
			...
			for row in reader:
				if header:
					header = False
                                        samples = [elem for elem in row[2:]]
					continue
				if firstcol_ensg:
					row[2:] = [float('nan') if elem=="NA" else float(elem) for elem in row[2:]]
					X.append(row[2:])
					features_ensg.append(row[0])
					features_hugo.append(row[1])
				else:
					row[1:] = [float('nan') if elem=="NA" else float(elem.strip()) for elem in row[1:]]
					X.append(row[1:])
	...
	#		self.test_acc_fn = theano.function([self.input_var, self.target_var], [self.test_acc], allow_input_downcast=True)
		...
		if is_train:
			if len(self.y_test)>0:
				...
				print("ERROR: TRAINING data but no test set available for evaluation")
...
if __name__ == '__main__':
	myargs = getargs()
        if(myargs.subcommand == 'test'):
                print("...TESTING input set")
                outdir=os.path.join(os.path.abspath(myargs.outdir[0]) + "/" + os.path.splitext(os.path.basename((myargs.samplename[0]).rsplit('.',1)[0]))[0] + "_TESTPRED_" + myargs.pref[0] + "_" + datetime.datetime.now().strftime('%Y-%m-%d'));
		...
		in_args = glob.glob(myargs.indir[0] + "/argparse_args.txt")[0]
		in_model = glob.glob(myargs.indir[0] + "/lasagne_bestparams.npz")[0]
		in_labdict = glob.glob(myargs.indir[0] + "/dataset_train/"+ "*/dict_labels*"+".txt")[0]
		in_trainidx = glob.glob(myargs.indir[0] + "/dataset_train/" + "*/trainingidxs.txt")[0]
		in_trainfeats = glob.glob(myargs.indir[0] + "/featurelist.txt")[0]
		...
		if myargs_test.infile[0] is None:
			print("Please provide an input file"); exit(0)
		else:
                        x_test, samples_test, features_ensg, features_test = readtestinput(datfile=myargs_test.infile[0], header_status=False, firstcol_ensg=True)
                
		##First subselect features, ordered in order of training set
		x_test = subset_features_x(x_test, features_test, features_train)
		if(len(samples_test) != x_test.shape[1]):
			samples_test  = ["unknown"] * x_test.shape[1]
		##Then normalize features according to training set
		myargs = argparse.Namespace(**myargs)
		if(myargs.normalize[0] == 1):
			print("Normalization is per sample, so not considering training dat")
			print("Normalization: {0}".format(myargs.normtype[0]))
		...
		if(myargs_test.makeblind[0] > 0):
			print("Randomly setting {0} fraction of features to 0".format(myargs_test.makeblind[0]))
