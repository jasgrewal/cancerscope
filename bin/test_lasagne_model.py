...
class lasagneNN(object):
	def __init__(self, modeldir):
		self.in_modeldir = modeldir
		
	def load_model(self,indir,is_image=False):
		...
		network = build_custom_mlp(input_var=input_var, is_image=is_image, n_out=n_class, num_features=n_in, depth=n_hiddenlayers, width=n_hidden, drop_hidden=0, drop_input=0)
	...
	def predict(self,X):
		X_normed = apply_norm_func(norm_func=self.normalization_input[0], xdat = X, bysamples=1)
